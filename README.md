...
<a href="https://ibb.co/JRQJRbSJ"><img src="https://i.ibb.co/Y7Zs7KVs/2026-08-05-183055.png" alt="屏幕截图 2026 08 05 183055" border="0"></a>
...
| 密度图        | 选择 | 折线图     | 关闭、直方图或折线图                     |
| 密度图宽度      | 数字 | 30      | 左侧密度图区域的宽度（px）                 |
| 直方图柱宽度     | 数字 | 5       | 每个密度柱（横条）的固定厚度（px），修改纵轴（时间轴）比例 |
| 折线图填充      | 开关 | 开启      | 开启后折线图下方会显示半透明的纯色填充（跟随密度图颜色）   |
...
> **密度图说明**：时间轴竖向排列（底部 = 歌开始，顶部 = 歌结束），柱长/折线长度 = 该区间音符数 / 全谱面最高音符数 × 可用宽度，达到全谱面最高密度的区间显示为上限颜色，白色游标指示当前播放位置。
